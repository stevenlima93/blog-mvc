...
if(isset($_GET['article_id']) && isset($_GET['action']) && $_GET['action'] == 'delete'){
    $imgSelect = $db->prepare('SELECT image FROM article WHERE id = ?');
    $imgSelect->execute([
        $_GET['article_id'] ]);

    $imgRecup = $imgSelect->fetch();
	$query = $db->prepare('DELETE FROM article WHERE id = ?');
	$result = $query->execute([$_GET['article_id']]);
    $imgRecup = $imgSelect->fetch();
    $pathDelete = '../img/article/';

    unlink($pathDelete . $imgRecup ['image']);
	//générer un message à afficher pour l'administrateur
    $deleteJoin = $db->prepare('DELETE FROM article_category WHERE article_id = ?');
    $deleteJoin->execute(array($_GET['article_id']));
